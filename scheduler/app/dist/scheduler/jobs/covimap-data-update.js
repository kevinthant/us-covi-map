...
const getData = url => {
  return new Promise((resolve, reject) => {
    rp(url).then(str => {
      csv().fromString(str).then(resolve).catch(reject);
    }).catch(reject);
  });
};
...
const CovimapDataUpdate = async jobData => {
  ...
  try {
    await conn.promise().execute(`CREATE TABLE IF NOT EXISTS \`state_cases\` (
    ...
    await conn.promise().execute('TRUNCATE TABLE `state_cases`;');
    let count = 0;
    await Promise.all(stateData.map(item => {
      count++;
      return conn.promise().execute('INSERT INTO `state_cases` (date, state, fips, cases, deaths) VALUES (?, ?, ?, ?, ?)', [item.date, item.state, item.fips, item.cases, item.deaths]);
    }));
    console.log(`Updated State data, total = ${count}`);
    await conn.promise().execute(`CREATE TABLE IF NOT EXISTS \`county_cases\` (
        \`date\` date DEFAULT NULL,
    ...
    await conn.promise().execute('TRUNCATE TABLE `county_cases`;');
    count = 0;
    await Promise.all(countyData.map(item => {
      count++;
      return conn.promise().execute('INSERT INTO `county_cases` (date, county, state, fips, cases, deaths) VALUES (?, ?, ?, ?, ?, ?)', [item.date, item.county, item.state, item.fips, item.cases, item.deaths]);
    }));
    conn.end();
