...
var _covimapDataUpdate = _interopRequireDefault(require("./jobs/covimap-data-update"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

let interval = null;
const registeredJobs = {
  [_covimapDataUpdate.default.name]: _covimapDataUpdate.default
};
const config = {};
...
module.exports = function Jobs({
  ...
  config.retentionTime = parseRetentionPeriod(retentionPeriod); //by default every one hour checking to clean up old records as per retention period given
...
function start({
  ...
  process.env.DEBUG && console.log("[Jobs] started");
...
function stop() {
  ...
  process.env.DEBUG && console.log("[Jobs] stopped");
}
...
async function schedule(due_at, jobFunc, ...argsJson) {
  try {
    ...
    /* istanbul ignore next */
    process.env.DEBUG && console.trace();
    throw error;
  }
  /* istanbul ignore next */


  process.env.DEBUG && console.log(`[Jobs] scheduling ${jobFunc.name}(${argsJson ? argsJson.map(x => JSON.stringify(x)).join(", ") : ""}) at ${due_at}`);
  return await config.storage.add({
    due_at,
    type: jobFunc.name,
    args: JSON.stringify(argsJson).replace("'", "''")
  });
}
...
async function cancel(jobId) {
  /* istanbul ignore next */
  process.env.DEBUG && console.log(`[Jobs] cancelling ${jobId}`);
  return await config.storage.remove(x => x.id == jobId);
}

async function query(predicate) {
  /* istanbul ignore next */
  process.env.DEBUG && console.log("[Jobs] querying jobs");
...
async function checkAndExecuteJobs() {
  const now = new Date();
  const jobs = await config.storage.find(x => {
    return (!x.run_at || x.cron) && new Date(x.due_at) < now;
  });
...
function runJob(job) {
  /* istanbul ignore next */
  process.env.DEBUG && console.log(`[Jobs] ${job.id} due_at ${new Date(job.due_at)} run_at ${job.run_at} ${job.type}`);
  ...
  jobArgs && jobArgs.length ? jobFunc(...jobArgs).then(successHandler).catch(errorHandler) : jobFunc().then(successHandler).catch(errorHandler);
}

function getNextDueDateForCron({
  value,
  endDateTime = null
}) {
  ...
  const cronInterval = _cronParser.default.parseExpression(value, {
