...
const readCsvRow = async (url, rowUpdateCallback, onFinishCallback) => {
  ...
  try {
    ...
    const readStream = fs.createReadStream(`/tmp/${filename}`).pipe(csv());
...
const csvDataUpdater = async ({
  ...
  await conn.promise().execute(schema);
  let count = 0;
  await readCsvRow(csvUrl, async item => {
    ...
    count++;
    await conn.promise().execute(updateQuery, getUpdateParams(item));
  }, () => {
    console.log(`Updated ${type} data, total = ${count}`);
  });
};

var _default = csvDataUpdater;
