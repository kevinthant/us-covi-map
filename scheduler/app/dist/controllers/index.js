...
var _jsonFile = _interopRequireDefault(require("./../scheduler/storage/json-file"));
...
const Jobs = require("../scheduler")({
  storage: new _jsonFile.default(_path.default.resolve(__dirname, './../../config/db.json')),
  ...
  addSchedule: (req, res) => {
    ...
    if (!registeredJobs[job]) {
      return res.status(400).json({
        message: `Job type ${job} passed is not valid or supported.`
      });
    }

    if (!after && !cron) {
      return res.status(400).json({
        message: 'Either "after" or "cron" property is required to schedule a task'
      });
    }
    ...
    if (after) {
      ...
      try {
        ...
        console.error(err);
        return res.status(400).json({
          message: `Problem with parsing Cron expression value: ${cron.value}`
        });
      }
    }
    ...
    if (!afterDate && !cron) {
      return res.status(400).json({
        message: 'Invalid after date string given'
      });
    } else if (!afterDate) {
      return res.status(400).json({
        message: 'Invalid cron setting given'
      });
    }

    if (!cron) {
      return Jobs.schedule(afterDate.toISOString(), registeredJobs[job], data).then(id => {
        res.json({
          id
        });
    ...
    return Jobs.scheduleCron(cron, registeredJobs[job], data).then(id => {
      res.json({
        id
      });
    }).catch(errorHandler(res));
