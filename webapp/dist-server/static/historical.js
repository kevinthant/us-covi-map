...
const getDdlOptions = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    Highcharts.getJSON(`/historical/drill-down-options.json${query}`, ({
      rows = null,
      error = false
    }) => {
      if (error) {
...
class HistoricalChartModel {
  constructor(options, stateOptions) {
    var self = this;
    this.selectedTickers = ko.observableArray([{
      key: this.constructor.US_TICKER_KEY
    }]);
    ...
    this.populateData = function () {
      ...
      if (state && !county) {
        ...
        newTicker = {
          key: self.constructor.US_TICKER_KEY
        };
      }
    ...
    this.refreshChart = selectedTickers => {
      getChartData(selectedTickers).then(({
        options
      }) => {
...
function initChart() {
  Promise.all([getChartData([{
    key: HistoricalChartModel.US_TICKER_KEY
  }]), getDdlOptions()]).then(([{
    options
  }, stateOptions]) => {
...
_onReady.onReady.then(initChart);